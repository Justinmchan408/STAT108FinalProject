...
The response variable, \textbf{price}, is the price of the car in our dataset. In order to be able to predict price, we performed some initial univariate analysis of price to observe its spread in the dataset.
...
\textbf{Price} seems unimodal meaning that there is one peak. It also seems to be skewed to the right where there are many datapoints that have \textbf{price} around 5,000-10,000 but there are a few outliers that have \textbf{price} greater than 25,000 dollars. To follow up with our analysis, we also created summary statistics for price to see if the statistics reflected the graph we observed.
...
Due to the clear relationships between \textbf{citympg}/\textbf{highwaympg}, \textbf{car height}/\textbf{car length}, and \textbf{car height}/\textbf{car width}, these interactions may be something to consider when creating the linear model. \textbf{Carwidth} and \textbf{carlength} seemed to have the weakest relationship but, should still be considered when making the model.

## Modeling Approach
The modeling approach we decided on was to select from the fourteen covariates that seemed to have a linear relationship with \textbf{price} from our bivariate analysis for our model. A multiple linear regression model will be used because the outcome of car price is continuous and there are several covariates that have a linear relationship with \textbf{price}. We wanted to find the AIC and BIC for each combination of covariates to determine which selection of covariates would be best to use in our linear model. This helps us to prevent our model from overfitting. We wanted to search all possible models so we used the method dredge() from the MuMIn package to be able to create all the possible models from our covariates and order them by AIC and BIC values.
...
Next we ran VIF on the model to check for multicollinearity. \textbf{carlength} had a VIF of 28.32 and \textbf{curbweight} had a VIF of 253.92 which are both > 10. We decided to remove curbweight because it has a much larger VIF than carlength.
...
$y = \beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + \beta_{3}X_{3} + \beta_{4}X_{4} + \beta_{5}X_{5} + \beta_{6}X_{6} + \beta_{7}X_{7} + \beta_{8}X_{8} + \beta_{9}X_{9} + \varepsilon$

$y = \beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + \beta_{3}X_{3} + \beta_{4}X_{4} + \beta_{5}X_{5} + \beta_{6}X_{6} + \beta_{7}X_{7} + \beta_{8}X_{8} + \beta_{9}X_{9} + \varepsilon$

Based on the table of the linear model above, the estimate intercept is telling us the price of a car with a car length of 0, engine size of 0, highwaympg of 0, horsepower of 0, and four wheel drive which is not realistic at all. The intercept estimate, -20177.318 does not tell us anything meaningful relative towards the data model. For the coefficients, -1262.343 for fwd in drivewheel and 1258.834 for rwd in drivewheel means that relative to the intercept of 4wd then these would be added to the new intercept for observations of fwd and rwd. The coefficient 98.617 for engine size means that the for one unit increase for the enginesize means that there is a general increase of 98.62 in car price. For the coefficient 35.703 for highwaympg generally means that for every increase in mile per gallon a car has on the highway, there would be an increase of an average of 35.70 dollars in car price. For coefficient of horsepower of 51.937, this means that for every increase in horsepower a car has, in general that there is an increase of about 51.94 in car price. The coefficient 84.491 for car length means that the for one unit increase for the carlength means that there is a general increase of 84.49 in car price.

## Limitations
There are a few limitations of the model that should be acknowledged when it comes to terms of car price prediction. To begin with, the univariate analysis shows some limits the range of data used to train the model for price prediction. For example, the multivariate linear regression model trains on the variable, car length which ranged from 140 to 200 units in our dataset so it would not be wise to use a car with a car length outside the range we trained the model with. In addition to car length, both engine size and horsepower had ranges from 0-200 and highway mpg 20-50 where the model would not be performing optimally.
